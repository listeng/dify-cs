...
const ChatWrapper = () => {
  ...
  const {
    ...
    appConfig,
    {
      inputs: (systemVariables.hideparams === '1' && systemVariables.isnew === '1'
        ? newConversationInputs
        : (currentConversationId ? currentConversationInputs : newConversationInputs)) as any,
  ...
  const inputDisabled = useMemo(() => {
    ...
    if (fileIsUploading)
      return true
    return false
  }, [allInputsHidden, inputsFormValue, inputsForms, systemVariables.hideparams])
  ...
  const doSend: OnSend = useCallback((message, files, isRegenerate = false, parentAnswer: ChatItem | null = null) => {
    ...
    const data: any = {
      query: message,
      files,
      inputs: shouldAlwaysUseUrlInputs ? newConversationInputs : (currentConversationId ? currentConversationInputs : newConversationInputs),
  ...
  }, [currentConversationId, currentConversationInputs, newConversationInputs, chatList, handleSend, appSourceType, appId, handleNewConversationCompleted, systemVariables.hideparams, systemVariables.isnew])

  useEffect(() => {
    setAutoSendCallback(doSend)
    return () => {
      setAutoSendCallback(null)
    }
