...
const ChatWrapper = () => {
  ...
  const inputDisabled = useMemo(() => {
    ...
    if (chatList.some(item => item.isAnswer && item.humanInputFormDataList && item.humanInputFormDataList.length > 0))
      return true
    return false
  }, [allInputsHidden, chatList, inputsFormValue, inputsForms, systemVariables?.hideparams])
  ...
  const [collapsed, setCollapsed] = useState(systemVariables?.hideparams === '1' || !!currentConversationId)
