...
const RunOnce: FC<IRunOnceProps> = ({
  ...
  useEffect(() => {
    ...
    promptConfig.prompt_variables.forEach((item) => {
      if (item.type === 'select')
        ...
        defaultInputs[item.key] = item.default || false
      else if (item.type === 'file')
        defaultInputs[item.key] = undefined
      else if (item.type === 'file-list')
        defaultInputs[item.key] = []
      else
        defaultInputs[item.key] = undefined
    })
    const preservedInputs = Object.fromEntries(
      Object.entries(inputsRef.current || {}).filter(([, value]) => value !== undefined),
    )
    onInputsChange({
      ...defaultInputs,
      ...preservedInputs,
  ...
  }, [isInitialized, onInputsChange, promptConfig.prompt_variables, inputsRef])
