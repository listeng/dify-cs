...
import { getRoutePath, getSigninPath } from '@/utils/route-utils'
...
function requiredWebSSOLogin(message?: string, code?: number) {
  ...
  const signinPathname = new URL(signinUrl, globalThis.location.origin).pathname

  if (globalThis.location.pathname === signinPathname)
    return
...
export const request = async<T>(url: string, options = {}, otherOptions?: IOtherOptions) => {
  try {
    ...
    if (errResp.status === 401) {
      ...
      const loginPath = getRoutePath('signin')
      const loginPathname = new URL(loginPath, globalThis.location.origin).pathname
      const loginUrl = `${globalThis.location.origin}${loginPath}`
      if (parseErr) {
      ...
      if (code === 'not_init_validated' && IS_CE_EDITION) {
        jumpTo(`${globalThis.location.origin}${getRoutePath('init')}`)
        return Promise.reject(err)
      }
      if (code === 'not_setup' && IS_CE_EDITION) {
        jumpTo(`${globalThis.location.origin}${getRoutePath('install')}`)
        return Promise.reject(err)
      }
      ...
      if (location.pathname !== loginPathname || !IS_CE_EDITION) {
        jumpTo(loginUrl)
        return Promise.reject(err)
      }
      if (!silent) {
