...
const normalizeBasePath = (pathname: string) => {
  const normalizedBasePath = env.NEXT_PUBLIC_BASE_PATH.replace(/^\/+|\/+$/g, '')
...
const wrapResponseWithXFrameOptions = (response: NextResponse, pathname: string) => {
  ...
  if (env.NEXT_PUBLIC_ALLOW_EMBED !== true && !normalizedPathname.startsWith('/chat') && !normalizedPathname.startsWith('/workflow') && !normalizedPathname.startsWith('/completion') && !normalizedPathname.startsWith('/webapp-signin'))
